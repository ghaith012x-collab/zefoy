    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                title = page.title()
                url = page.url
                log(f"Page title='{title}' url={url}")
                emit(q, 1, f"Page ready: '{title}' @ {url}")
            except Exception as e:
                log(f"Could not read page title: {e}")
